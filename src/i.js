...
export default class extends PureComponent {
  ...
  simulateDrawingLines = ({ lines, immediate }) => {
    ...
    lines.forEach(line => {
      ...
      if (immediate) {
        ...
        this.points = points;
        this.saveLine({ brushColor, brushRadius, loadedAfterSocketResponse:true }); 
        // here i (rishabh sharma) have passed an extra key to mmake sure that 
        // i recognize in onChange method wether it is called afetr server update or 
        //mouse raised
        return;
      }
  ...
  saveLine = ({ brushColor, brushRadius,loadedAfterSocketResponse } = {}) => {
    ...
    this.triggerOnChange(loadedAfterSocketResponse);
  };

  triggerOnChange = (loadedAfterSocketResponse) => {
    let a =this;
    a['loadedAfterSocketResponse']=loadedAfterSocketResponse;
